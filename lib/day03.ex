defmodule Day03 do
  ...
  def extract_number_ranges(str, acc \\ []) do
    ...
    acc = cond do
      ...
      item != nil -> extract_number_ranges(str, [ item | acc] )
      true -> acc
      end
    acc
  end
  ...
  def get_symbol_indices_for_row(row, symbol_indices_by_row) do
    ...
    symbol_indices = Enum.filter(row-1..row+1, fn x ->
  ...
  def keep_filter_part?(item, symbol_indices_by_row) do
    ...
    keep = cond do
      symbols == [] -> true
      true -> !Enum.any?(symbols, fn symbol_index ->
  ...
  *  any number adjacent to a symbol, even diagonally, is a "part number" and should be included in your sum
  ...
  def sum_numbers_adjacent_to_symbols(fileName) do
    ...
    number_indices = File.read!(fileName)
    |> String.split("\n")
    ...
    symbol_indices_by_row = File.read!(fileName)
    |> String.split("\n")
    ...
    kept = Enum.filter(number_indices, fn x ->
      keep_filter_part?(x, symbol_indices_by_row)
    end)
    IO.puts("Kept: #{inspect(kept)}")
    Enum.map(kept, fn x -> x.item.value end)
    |> Enum.sum()
